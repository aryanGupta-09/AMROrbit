...
export default function Plots() {
  ...
  const [refTip, setRefTip] = useState({
    x: 0,
    y: 0,
    text: "",
    visible: false,
  ...
  useEffect(() => {
    if (!isYearSet) {
      // Only start the loop if it's not manually changed
      let currentIndex = 0;
  ...
  if (
    !firstRender &&
    files.years &&
    files.years.length === 0 &&
    files.countries &&
    files.countries.length === 0
  ) {
    return (
      <VisualizationBox
        heading="Continue Visualization"
        text="Sorry, no data is available for this drug-bug combination in this country."
    ...
    if (active && payload && payload.length) {
      ...
      const index =
        selectedIndex === undefined
          ? data.findIndex((item) => item.label === dataLabel)
          : selectedIndex;
  ...
  return options.country === null || options.country === "All"
    ? (() => {
        const { maxX, maxY, minX, minY } = files.years.reduce(
          (acc, year) => {
        ...
        return (
          <div>
            ...
            <div className="flex flex-wrap justify-around gap-y-7">
              {(() => {
                const year = files.years.find(
                  (year) => Number(year.year) === selectedYear
                );
                if (!year) return null;
                ...
                return (
                  <div style={{ width: "90%" }} className="flex flex-col">
                    ...
                    >
                      <div
                        className="bg-[#f1f2f7] rounded-xl shadow-lg flex-grow"
                        style={{ height: "70vh" }}
                      >
                            ...
                            {isYearSet && (
                              <ReferenceLine
                                ...
                                onMouseEnter={(e) =>
                                  handleShowRefTip(
                                    e,
                                    `Median Intercept: ${year.median_intercept.toFixed(
                                      2
                                    )}`,
                                    "green"
                                  )
                            ...
                            {isYearSet && (
                              <ReferenceLine
                                ...
                                onMouseEnter={(e) =>
                                  handleShowRefTip(
                                    e,
                                    `Median Slope: ${year.median_slope.toFixed(
                                      2
                                    )}`,
                                    "red"
                                  )
                              ...
                              {data.map((entry, index) => {
                                ...
                                return (
                                  <Cell
                                    ...
                                    onClick={() =>
                                      handleCountryClick(modifiedEntry.label)
                                    }
                                  />
                                );
                              })}
                      ...
                      <div
                        style={{ width: "15%" }}
                        className="flex flex-col gap-y-3 text-xs sm:text-sm lg:text-base xl:text-lg 2xl:text-xl 3xl:text-2xl"
                      >
                    ...
                    <div className="flex items-center pl-3 mt-3">
                      <Image
                        src={infoIcon}
                        alt="Info Icon"
                        className="w-4 h-4 sm:w-6 sm:h-6 lg:w-7 lg:h-7 2xl:w-8 2xl:h-8 3xl:w-10 3xl:h-10 filter invert-[100%]"
                      />
                      <span className="ml-2 text-white text-sm sm:text-base lg:text-lg 2xl:text-xl 3xl:text-3xl">
                        Hover over a country from the legend to focus on it, or
                        click to view its specific trends.
                      </span>
                    </div>
                  </div>
                );
    ...
    : (() => {
        const selectedCountryIndex = files.countries.findIndex(
        ...
        return (
          <div
            style={{ width: "90%" }}
            className="flex flex-row justify-around gap-x-3 mx-auto mt-7"
          >
                  ...
                  {selectedCountry.years
                    ...
                    .map((year, index) => {
                      ...
                      return (
                        <div
                          style={{ height: "70vh" }}
                          className="embla__slide bg-[#f1f2f7] rounded-xl shadow-lg flex justify-center items-center relative"
                          key={index}
                        >
                          <div
                            className={`absolute top-3 right-3 z-10 p-2 rounded-lg text-white text-lg 2xl:text-xl 3xl:text-2xl`}
                            style={{
                              backgroundColor:
                                colors[selectedCountryIndex % colors.length],
                            }}
                          >
            ...
            >
              ...
              <div
                className="bg-[#4F6077] text-white text-center rounded-xl shadow-lg p-2 h-fit cursor-pointer text-xs sm:text-sm lg:text-base xl:text-lg 2xl:text-xl 3xl:text-2xl"
                onClick={() => handleCountryClick("All")}
              >
                Back to all <span className="hidden xs:inline">countries</span>
              </div>
            </div>
          </div>
        );
