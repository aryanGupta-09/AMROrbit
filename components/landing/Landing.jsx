...
import years from "@/public/scorecards/Klebsiella pneumoniae/Blood/Amikacin_Klebsiella pneumoniae_year.json";
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      currentIndexRef.current =
        (currentIndexRef.current + 1) % sortedYears.length;
    }, 500);

    return () => clearInterval(interval); // Cleanup interval on component unmount
  }, [sortedYears]);

  const { maxX, maxY, minX, minY } = years.reduce(
    (acc, year) => {
  ...
  return (
    <div className="flex flex-col gap-y-5 2xl:gap-y-6 3xl:gap-y-7 justify-center items-center my-6 mb-14 mx-auto w-10/12">
      <h1 className="text-4xl lg:text-5xl 3xl:text-6xl 4xl:text-7xl font-bold text-gray-100 text-center">
        Our World in <span className="text-[#BAC0DF]">AMR</span>
      </h1>
      <ResponsiveContainer
        className="bg-[#f1f2f7] rounded-xl shadow-lg"
        width="100%"
        height={is3xl ? 800 : is2xl ? 600 : isMd ? 500 : 350}
      >
        <ScatterChart
          margin={{
            top: 20,
            ...
            {data.map((entry, index) => {
              ...
              return (
                <Cell
                  ...
                  opacity={modifiedEntry.label === "Colombia" ? 1 : 0.3}
                />
              );
            })}
          </Scatter>
