...
export default function SyntheticDatasetCreation() {
    ...
    const [preserveProportions, setPreserveProportions] =
        useState("country_year");
        ...
        fetch(`/api/test-model/mapping?id=${id}`)
            ...
            .then((res) => {
                if (res.success) {
                    ...
                    toast.error("Failed to fetch data. Reason: " + res.message);
                }
            })
            .catch((err) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/generate-synthetic-dataset`,
                {
                    method: "POST",
                    headers: {
            ...
            <main className="flex-1 max-w-5xl mx-auto p-10">
                <div className="flex items-center mb-10">
                    <button
                        ...
                        onClick={() =>
                            router.push(
                                `/test-model/dashboard/result?id=${id}&proc=0`
                            )
                        }
                    >
                        &larr;
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="mb-5">
                                ...
                                <input
                                    ...
                                    value={nTotal}
                                    onChange={(e) =>
                                        setNTotal(Number(e.target.value))
                                    }
                                    required
                            ...
                            <div
                                className={`flex items-center gap-2 mb-5 transition-opacity ${
                                    preserveProportions === "none"
                                        ? "opacity-50"
                                        : ""
                                }`}
                            >
                                ...
                                <label htmlFor="anonymize" className="text-sm">
                                    Anonymize Study, Country, and State
                                </label>
                            </div>
                            <div className="flex items-center gap-2 mb-7">
                                <input
                                    ...
                                    onChange={(e) =>
                                        setGeneratePlots(e.target.checked)
                                    }
                                />
                                <label
                                    htmlFor="generate_plots"
                                    className="text-sm"
                                >
